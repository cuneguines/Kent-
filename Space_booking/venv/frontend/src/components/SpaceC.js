...
const SpaceC = () => {
  ...
  useEffect(() => {
    setSpace('Space C'); // Set the initial value for selectedSpace
  }, []);
  ...
  const handleSpaceButtonClick = (spaced) => {
    ...
    const filteredBookings = new_bookings.filter(booking => booking.space === 'Space C');
    console.log(filteredBookings);
  ...
  setStartTime(selectedStartTime);
    fetch(`/available-endtimes/${selectedDate}/${selectedStartTime}/${'Space C'}`)
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleDateChange = (event) => {
    ...
    fetch(`/available-times/${selectedDate}/${'space C'}`)
      .then((response) => response.json())
      .then((data) => {
        setAvailableTimes(data);
  ...
  useEffect(() => {
    fetch(`/api/data/${'Space C'}`)
      .then(response => response.json())
      .then(data => {
        setData(data); setFilteredBookings(data); console.log('datai', data);

        const filteredBookings = data.filter(booking => booking.space === 'Space C');
        setFilteredBookings(filteredBookings);
              ...
              <tr key={timeSlot}>
                ...
                {daysOfWeek.map(day => {
                  const booking = filteredBookings.find(
                    booking => booking.Date === day && booking.startTime === timeSlot.split("-")[0] && booking.space === 'Space C'
                  );

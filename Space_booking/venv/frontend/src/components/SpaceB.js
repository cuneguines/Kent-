...
const SpaceB = () => {
  ...
  useEffect(() => {
    setSpace('Space B'); // Set the initial value for selectedSpace
  }, []);
  ...
  const handleSpaceButtonClick = (spaced) => {
    ...
    const filteredBookings = new_bookings.filter(booking => booking.space === 'Space B');
    console.log(filteredBookings);
  ...
  useEffect(() => {
    fetch(`/api/data/${'Space B'}`)
      .then(response => response.json())
      .then(data => {
        setData(data); setFilteredBookings(data); console.log('datai', data);

        const filteredBookings = data.filter(booking => booking.space === 'Space B');
        setFilteredBookings(filteredBookings);
              ...
              <tr key={timeSlot}>
                ...
                {daysOfWeek.map(day => {
                  const booking = filteredBookings.find(
                    booking => booking.Date === day && booking.startTime === timeSlot.split("-")[0] && booking.space === 'Space B'
                  );

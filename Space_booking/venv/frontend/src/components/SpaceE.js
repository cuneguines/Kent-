...
const SpaceE = () => {
  ...
  useEffect(() => {
    setSpace('Space E'); // Set the initial value for selectedSpace
  }, []);
  ...
  const handleSpaceButtonClick = (spaced) => {
    ...
    const filteredBookings = new_bookings.filter(booking => booking.space === 'Space E');
    console.log(filteredBookings);
  ...
  setStartTime(selectedStartTime);
    fetch(`/available-endtimes/${selectedDate}/${selectedStartTime}/${'Space E'}`)
      .then((response) => response.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch(`/api/data/${'Space E'}`)
      .then(response => response.json())
      .then(data => {
        setData(data); setFilteredBookings(data); console.log('datai', data);

        const filteredBookings = data.filter(booking => booking.space === 'Space E');
        setFilteredBookings(filteredBookings);
              ...
              <tr key={timeSlot}>
                ...
                {daysOfWeek.map(day => {
                  const booking = filteredBookings.find(
                    booking => booking.Date === day && booking.startTime === timeSlot.split("-")[0] && booking.space === 'Space E'
                  );
